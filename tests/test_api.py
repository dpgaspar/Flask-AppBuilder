...
class APITestCase(FABTestCase):
    ...
    def test_auth_authorization(self):
        ...
        uri = f"api/v1/model1apirestrictedpermissions/10"
    ...
    def test_get_item_base_filters(self):
        ...
        with model1_data(self.appbuilder.session, 4) as models:
            ...
            rv = self.auth_client_get(client, token, f"api/v1/model1apifiltered/{4}")
            self.assertEqual(rv.status_code, 200)
    ...
    def test_get_list_filters_m_m(self):
        ...
        with model_mm_parent_data(self.appbuilder.session, 2, 4):
            arguments = {
                API_FILTERS_RIS_KEY: [
                    {"col": "children", "opr": "rel_m_m", "value": [2, 3, 4]}
                ]
            }
            uri = f"api/v1/modelmmapi/?{API_URI_RIS_KEY}={prison.dumps(arguments)}"
            rv = self.auth_client_get(client, token, uri)
    ...
    def test_create_item_with_enum_validation(self):
        """
        REST Api: Test validation on create item with enum
        """
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        item = dict(enum2="e9")
    ...
    def test_create_item_with_enum_inline_validation(self):
        """
        REST Api: Test validation on create item with inline enum
        """
        client = self.app.test_client()
        token = self.login(client, USERNAME_ADMIN, PASSWORD_ADMIN)
        item = dict(enum1="e9")
        ...
        self.assertEqual(rv.status_code, 422)
        self.assertEqual(
            data, {"message": {"enum1": ["Must be one of: e1, e2, e3, e4."]}}
        )
    ...
    def test_create_item_mm_field(self):
        ...
        with model_mm_parent_data(self.appbuilder.session, 1, 4):
