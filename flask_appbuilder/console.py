...
def import_application(app_package, appbuilder):
    ...
    try:
        ...
        click.echo(
            click.style(
                'Was unable to import {0} Error: {1}'.format(app_package, e),
                fg='red'))
        exit(3)
...
@click.option(
    '--app', default='app', help='Your application init directory (package)')
@click.option(
    '--appbuilder', default='appbuilder', help='your AppBuilder object')
@click.option(
    '--username',
    default='admin',
    prompt='The username',
...
def reset_password(app, appbuilder, username, password):
    ...
    if not user:
        ...
        click.echo(
            click.style('User {0} reseted.'.format(username), fg='green'))
...
@click.password_option()
def create_admin(app, appbuilder, username, firstname, lastname, email,
                 password):
    """
    ...
    user = _appbuilder.sm.add_user(username, firstname, lastname, email,
                                   role_admin, password)
    if user:
        click.echo(
            click.style(
                'Admin User {0} created.'.format(username), fg='green'))
    else:
...
def version(app, appbuilder):
    ...
    click.echo(
        click.style(
            'F.A.B Version: {0}.'.format(_appbuilder.version),
            bg='blue',
            fg='white'))
...
def list_users(app, appbuilder):
    ...
    for view in _appbuilder.baseviews:
        click.echo('View:{0} | Route:{1} | Perms:{2}'.format(
...
def list_users(app, appbuilder):
    ...
    for user in _appbuilder.sm.get_all_users():
        click.echo('username:{0} | email:{1} | role:{2}'.format(
...
@click.option(
    ...
    '-k',
    multiple=True,
    default=['lazy_gettext', 'gettext', '_', '__'])
def babel_extract(config, input, output, target, keywords):
    ...
    click.echo(
        click.style(
            'Starting Extractions config:{0} input:{1} output:{2} keywords:{3}'.
            format(config, input, output, keywords),
            fg='green'))
    keywords = ' -k '.join(keywords)
    os.popen('pybabel extract -F {0} -k {1} -o {2} {3}'.format(
        config, keywords, output, input))
    click.echo(
        click.style('Starting Update target:{0}'.format(target), fg='green'))
    os.popen('pybabel update -N -i {0} -d {1}'.format(output, target))
    click.echo(
        click.style('Finish, you can start your translations', fg='green'))
...
def babel_compile(target):
    ...
    """
    click.echo(
        click.style('Starting Compile target:{0}'.format(target), fg='green'))
...
def create_app(name, engine):
    ...
    try:
        ...
        click.echo(
            click.style(
                'Downloaded the skeleton app, good coding!', fg='green'))
        return True
    except Exception as e:
        ...
        if engine.lower() == 'sqlalchemy':
            click.echo(
                click.style(
                    'Try downloading from {0}'.format(SQLA_REPO_URL),
                    fg='green'))
        elif engine.lower() == 'mongoengine':
            click.echo(
                click.style(
                    'Try downloading from {0}'.format(MONGOENGIE_REPO_URL),
                    fg='green'))
        return False
...
@click.option(
    ...
    help=
    "Your addon name will be prefixed by fab_addon_, directory will have this name"
)
def create_addon(name):
    ...
    try:
        ...
        click.echo(
            click.style(
                'Downloaded the skeleton addon, good coding!', fg='green'))
        return True
    except Exception as e:
        click.echo(click.style('Something went wrong {0}'.format(e), fg='red'))
        return False
...
@click.option(
    '--static_folder',
    default='app/static',
    help='Your projects static folder')
def collect_static(static_folder):
    ...
    try:
        shutil.copytree(appbuilder_static_path,
                        os.path.join(app_static_path, 'appbuilder'))
    except Exception as e:
        click.echo(
            click.style(
