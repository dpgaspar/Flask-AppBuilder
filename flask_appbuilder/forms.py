...
from wtforms import (BooleanField, StringField, TextAreaField, IntegerField,
                     FloatField, DateField, DateTimeField, DecimalField)
from .fields import QuerySelectMultipleField, QuerySelectField, EnumField

from wtforms import validators
from .fieldwidgets import (BS3TextAreaFieldWidget, BS3TextFieldWidget,
...
class GeneralModelConverter(object):
    ...
    def _convert_col(self, col_name, label, description, lst_validators,
        ...
        if self.datamodel.is_relation(col_name):
            if self.datamodel.is_relation_many_to_one(col_name) or \
                ...
                return self._convert_many_to_one(col_name, label, description,
                                                 lst_validators,
                                                 filter_rel_fields, form_props)
                ...
                return self._convert_many_to_many(
                    col_name, label, description, lst_validators,
                    filter_rel_fields, form_props)
            else:
                log.warning("Relation {0} not supported".format(col_name))
        else:
            return self._convert_simple(col_name, label, description,
                                        lst_validators, form_props)
