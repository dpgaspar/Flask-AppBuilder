...
class FlaskTestCase(unittest.TestCase):
    def setUp(self):
        ...
        try:
            self.appbuilder.sm.add_user('admin', 'admin', 'user',
                                        'admin@fab.org', role_admin, 'general')
        except:
            pass
    ...
    def test_sec_reset_password(self):
        ...
        rv = client.get(
            '/users/action/resetmypassword/{0}'.format(user.id),
            follow_redirects=True)
        data = rv.data.decode('utf-8')
        ...
        rv = client.get(
            '/users/action/resetpasswords/{0}'.format(user.id),
            follow_redirects=True)
        data = rv.data.decode('utf-8')
        ok_("Reset Password Form" in data)
        rv = client.post(
